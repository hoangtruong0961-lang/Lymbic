...
const MainLayout: React.FC<MainLayoutProps> = ({ children }) => {
  ...
  return (
    <MotionConfig reducedMotion={visualEffects ? 'never' : 'always'}>
      {/* 
        Sử dụng h-[100dvh] để tương thích tốt nhất trên thiết bị di động (IOS Safari/Chrome).
        Áp dụng safe-area-padding từ MOBILE_CONFIG.
      */}
      <div 
        className="relative w-full h-full min-h-[100dvh] bg-[#e6ebf4] dark:bg-[#0b1329] text-slate-800 dark:text-slate-200 overflow-hidden selection:bg-mystic-accent selection:text-mystic-900 font-sans transition-colors duration-300"
        style={{ 
